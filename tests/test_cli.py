...
def test_diff_demo_exits_one(tmp_path, root):
    code = main([
        "diff", str(root / "fixtures" / "pre"), str(root / "fixtures" / "post"),
        "--intent", str(root / "intents" / "demo.yaml"),
...
def test_diff_clean_exits_zero(tmp_path, root):
    code = main([
        "diff", str(root / "fixtures" / "pre"), str(root / "fixtures" / "pre"),
        "--intent", str(root / "intents" / "demo.yaml"),
        "--report", str(tmp_path / "report"),
    ])
    assert code == 0
...
def test_invalid_input_cli_is_exit_two_without_traceback(tmp_path, root, capsys):
    assert main([
        "diff", str(tmp_path), str(tmp_path), "--intent", str(root / "intents/demo.yaml"),
        "--report", str(tmp_path / "report"),
    ]) == 2
